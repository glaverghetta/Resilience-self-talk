    ...
    void Update()
    {
        ...
        switch (_currentState)
        {
            ...
            case TowerState.REMOVING:
                BlockLastRows();
                break;
            /* case TowerState.WAITING:             // OLD VERSION !
                 if (waitTimer < waitDelay)
                 {
                     waitTimer += Time.deltaTime;
                 } else if (IsTowerSleeping())      // Notes: waiting for every block's Rigidbody to sleep can take an extremely long time, or very little time at all. This is what
                                                    // caused the game to become unresponsive sometimes.
                 {
                     print("tower is sleeping");        
                     _currentState = State;
                     OnStateChanged();
                 }
                 break;*/
